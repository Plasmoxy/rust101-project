...
//! Code by sgasse on infercam onnx project
//! https://github.com/sgasse/infercam_onnx/blob/main/infer_server/src/nn.rs
//!
use image::RgbImage;
use ndarray::s;
...
use crate::core::{download_file, Bbox, DynError};
type Error = DynError;
...
impl UltrafaceModel {
    ...
    pub async fn new(variant: UltrafaceVariant, max_iou: f32, min_confidence: f32) -> Result<Self, Error> {
    ...
    fn preproc(&self, input: RgbImage) -> TValue {
        ...
        let tensor: Tensor = tract_ndarray::Array4::from_shape_fn((1, 3, self.height as usize, self.width as usize), |(_, c, y, x)| {
    ...
    fn postproc(&self, raw_nn_out: NnOut) -> Result<Vec<(Bbox, f32)>, Error> {
        // Extract confidences
        let confidences = raw_nn_out[0].to_array_view::<f32>()?.slice(s![0, .., 1]).to_vec();

        // Extract relative coordinates of bounding boxes
        let bboxes: Vec<f32> = raw_nn_out[1].to_array_view::<f32>()?.iter().cloned().collect();
...
fn non_maximum_suppression(mut sorted_bboxes_with_confidences: Vec<(&Bbox, &f32)>, max_iou: f32) -> Vec<(Bbox, f32)> {
